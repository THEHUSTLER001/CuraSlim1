// Updated src/components/History.js with French translations and language prop support
import { useEffect, useState } from 'react';
import { Table, Button, Form, Modal, Pagination } from 'react-bootstrap';
import { FaSearch, FaSortAlphaDown, FaSortAlphaUp, FaEye, FaUndo } from 'react-icons/fa';
import axios from 'axios';
import './History.css';

function History({ language = 'fr' }) {
  ...
  const texts = {
    fr: {
      ...
      view: 'Voir',
      restore: 'Restaurer',
      confirm: 'Restaurer vers les commandes en attente ?',
    ...
    en: {
      ...
      view: 'View',
      restore: 'Restore',
      confirm: 'Restore to pending orders?',
  ...
  const restoreOrder = (id) => {
    if (window.confirm(currentTexts.confirm)) {
      axios.put(`/api/admin/orders/${id}`, { status: 'pending' }).then(() => fetchOrders(search, currentPage, sort));
    }
  };
            ...
            {orders.map((o) => (
              <tr key={o.id}>
                ...
                <td className="action-buttons">
                  ...
                  <Button variant="success" size="sm" onClick={() => restoreOrder(o.id)}>
                    <FaUndo className="me-1" /> {currentTexts.restore}
                  </Button>
                </td>
              </tr>
            ))}
