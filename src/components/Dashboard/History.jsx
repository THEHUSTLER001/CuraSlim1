// Updated src/components/History.js to import CSS
...
function History() {
  ...
  const fetchOrders = (q) => {
    axios
      .get('/api/admin/orders', { params: { q, limit: 10000, status: 'completed' } })
  ...
  const restoreOrder = (id) => {
    if (window.confirm('Restore to orders?')) {
      axios.put(`/api/admin/orders/${id}`, { status: 'pending' }).then(() => fetchOrders(search));
    }
  };
